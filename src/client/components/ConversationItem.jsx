...
export default function ConversationItem({ thread, isSelected, onSelect, accounts = [] }) {
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className={`w-full flex items-center gap-3 px-3 py-3 min-h-[82px] cursor-pointer text-left transition-colors border-b border-[var(--color-border)] ${
        isSelected
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2 mb-0.5">
          <span className={`text-sm truncate ${unreadCount > 0 ? 'font-bold' : 'font-semibold'} text-[var(--color-text-primary)]`}>
            {title}
          </span>
          <span className="text-xs text-[var(--color-text-muted)] flex-shrink-0 font-mono">{formatTime(thread.last_activity)}</span>
        </div>
        <p className={`text-xs truncate mb-1.5 ${unreadCount > 0 ? 'text-[var(--color-text-secondary)] font-medium' : 'text-[var(--color-text-muted)]'}`}>
          {preview}
        </p>
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            {showAccountName && (
              <span className="text-[11px] text-[var(--color-text-muted)] truncate max-w-[120px]" title={accountName}>
                {accountName}
              </span>
            )}
          </div>
          {unreadCount > 0 && (
            <span className="bg-[var(--color-accent)] text-[var(--color-text-on-accent)] text-xs rounded-full min-w-5 h-5 px-1.5 flex items-center justify-center font-bold shadow-sm">
              {unreadCount}
            </span>
          )}
        </div>
