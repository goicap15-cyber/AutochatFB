...
import { Check, ChevronRight, Eye, Plus, RefreshCw, Trash2, UserPlus, Users, X } from 'lucide-react';
      ...
      <div className="mx-auto max-w-6xl space-y-4 p-6">
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center justify-between gap-4 border-b border-slate-200 px-5 py-4">
            ...
            <button type="button" onClick={() => setShowCreate(true)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">
                ...
                <tbody className="divide-y divide-slate-100">
                  {data.employees.map((employee, index) => (
                    <tr key={employee.id} className="align-top hover:bg-slate-50/70">
                      ...
                      <td className="px-5 py-4">
                        <button type="button" title="Xem danh sách tài khoản Facebook đã gán" onClick={() => { setAssignmentViewMode('view'); setAssigningEmployee(employee); }} className="group inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">
                      ...
                      <td className="px-5 py-4">
                        <button type="button" onClick={() => { setAssignmentViewMode('edit'); setAssigningEmployee(employee); }} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-200 px-3 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-50">
                          <Plus size={15} /> Gán tài khoản
                        </button>
                      </td>
                      <td className="px-5 py-4 text-right">
                        <button type="button" title="Xóa nhân viên" aria-label={`Xóa nhân viên ${employee.username}`} disabled={busy} onClick={() => remove(employee)} className="inline-grid h-9 w-9 place-items-center rounded-lg border border-red-200 text-red-600 hover:bg-red-50 disabled:opacity-50"><Trash2 size={16} /></button>
                      </td>
                    </tr>
                  ))}
                  {!data.employees.length && (
                    <tr><td colSpan="5" className="px-5 py-16 text-center text-sm text-slate-500">Chưa có nhân viên. Bấm “Thêm nhân viên” để tạo tài khoản đầu tiên.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
